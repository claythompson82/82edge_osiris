...
def get_orchestrator_cmd_and_pid():
    ...
    try:
        ...
        result_pid = subprocess.run(pgrep_cmd, capture_output=True, text=True, check=False)
        ...
        for pid in pids:
            ...
            try:
                ...
                result_cmd = subprocess.run(ps_cmd, capture_output=True, text=True, check=True)
                command = result_cmd.stdout.strip()

                # Verify if the command indeed runs the orchestrator script
                if ORCHESTRATOR_SCRIPT_PATH in command and "python" in command.split()[0].lower():
                    log_message(f"Found orchestrator PID: {pid} with command: {command}")
                    return command, pid.strip()
            except subprocess.CalledProcessError as e:
                log_message(f"Error checking command for PID {pid}: {e}. Output: {e.stderr}")
            except Exception as e:
                log_message(f"An unexpected error occurred while checking command for PID {pid}: {e}")
        
        log_message(f"Could not confirm exact command for PIDs {pids} matching {ORCHESTRATOR_SCRIPT_PATH}.")
        return None, None

    except subprocess.CalledProcessError as e:
        log_message(f"Error finding orchestrator PID with pgrep: {e}. Stderr: {e.stderr}")
        return None, None
    except Exception as e:
        log_message(f"An unexpected error occurred in get_orchestrator_cmd_and_pid: {e}")
        return None, None
...
def restart_llm_sidecar():
    ...
    try:
        ...
        service_id = subprocess.run(ps_command, capture_output=True, text=True, cwd=REPO_ROOT, check=False)

        if service_id.returncode != 0 or not service_id.stdout.strip():
            log_message(f"llm-sidecar service not found or 'docker-compose ps' failed. Error: {service_id.stderr}")
            ...
            for cf in common_compose_files:
                if os.path.exists(cf):
                    ps_command_with_file = ['docker-compose', '-f', cf, 'ps', '-q', 'llm-sidecar']
                    log_message(f"Trying with compose file: {' '.join(ps_command_with_file)}")
                    service_id = subprocess.run(ps_command_with_file, capture_output=True, text=True, cwd=REPO_ROOT, check=False)
                    if service_id.returncode == 0 and service_id.stdout.strip():
                        compose_file_to_use = cf
                        break
            
            if not compose_file_to_use:
                log_message("Could not find a valid docker-compose file for llm-sidecar. Cannot restart.")
                return

            restart_command = ['docker-compose', '-f', compose_file_to_use, 'restart', 'llm-sidecar']
        else:
            restart_command = ['docker-compose', 'restart', 'llm-sidecar']


        log_message(f"Executing: {' '.join(restart_command)}")
        result = subprocess.run(restart_command, capture_output=True, text=True, cwd=REPO_ROOT, check=True)
        ...
        log_message("Error: docker-compose command not found. Is it installed and in PATH?")
    except Exception as e:
...
def restart_orchestrator():
    ...
    if pid:
        try:
            ...
            log_message(f"Failed to kill orchestrator (PID: {pid}): {e}. It might have already exited.")
        except Exception as e:
            log_message(f"An unexpected error occurred while killing orchestrator PID {pid}: {e}")
    else:
        log_message("Orchestrator PID not found. Will attempt to start if command is known (e.g. from a previous run or default).")
    ...
    if original_command:
        ...
        try:
            ...
            with open(os.devnull, 'wb') as devnull:
                subprocess.Popen(cmd_parts, cwd=REPO_ROOT, stdout=devnull, stderr=devnull, start_new_session=True)
            log_message(f"Orchestrator restart initiated with command: {' '.join(cmd_parts)}")

        except Exception as e:
            log_message(f"Failed to restart orchestrator with command '{original_command}': {e}")
    else:
        log_message("Original command for orchestrator not found. Cannot restart.")
        log_message(f"Consider starting it manually if needed, e.g., 'cd {REPO_ROOT} && python {ORCHESTRATOR_SCRIPT_PATH} --your-args &'")
...
def main():
    ...
    log_message(f"Sleep interval between restarts: {MIN_SLEEP_SECONDS} to {MAX_SLEEP_SECONDS} seconds.")

    if ORCHESTRATOR_SCRIPT_PATH.startswith('/'):
        log_message(f"Warning: ORCHESTRATOR_SCRIPT_PATH ('{ORCHESTRATOR_SCRIPT_PATH}') is absolute. "
    ...
    try:
        while True:
            ...
            log_message(f"Selected service to restart: {selected_service} at {current_time}")
