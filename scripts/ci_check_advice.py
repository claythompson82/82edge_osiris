...
def main():
    ...
    parser.add_argument('--db-path', type=str, required=True, help="Path to the LanceDB database directory.")
    parser.add_argument('--table-name', type=str, default="advice", help="Name of the table to check.")
    parser.add_argument('--min-entries', type=int, default=1, help="Minimum number of entries expected.")
    parser.add_argument('--retries', type=int, default=3, help="Number of retries to check for advice.")
    parser.add_argument('--retry-delay', type=int, default=10, help="Delay in seconds between retries.")
    ...
    if not os.listdir(args.db_path): # Check if the directory is empty
        print(f"Error: LanceDB path '{args.db_path}' is empty. Table '{args.table_name}' likely does not exist.")
        sys.exit(1)


    for attempt in range(args.retries):
        try:
            print(f"Attempt {attempt + 1}/{args.retries} to connect and check table '{args.table_name}'...")
            # Connect to LanceDB
            ...
            if args.table_name not in table_names:
                print(f"Table '{args.table_name}' not found in database. Available: {table_names}")
            ...
            if count >= args.min_entries:
                print(f"Success: Found {count} entries, which is >= minimum expectation of {args.min_entries}.")
                sys.exit(0) # Success
            else:
                print(f"Failure: Found {count} entries, but expected at least {args.min_entries}.")
                if attempt < args.retries - 1:
                    print(f"Retrying in {args.retry_delay} seconds...")
                    time.sleep(args.retry_delay)
                else:
