...
async def meta_loop():
    ...
    while True:
        ...
        if not accepted:
            log.warning(f"Patch for {patch.get('target')} was not approved by verifier (score: {verification_score}), skipping application.")
        ...
        if _apply_patch(patch):
            ...
            new_r = sum(proofable_reward(t, patch.get("after")) for t in traces) # Pass patch content to reward
            if new_r >= 0:   # simple non-regression gate for now
                REDIS.lpush(APPLIED_LOG, json.dumps(patch | {"reward": new_r, "verification_score": verification_score}))
                log.info("Patch applied ✔️  reward=%.4f, verification_score=%.2f", new_r, verification_score)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--once", action="store_true", help="Run the meta-loop only once.")
    ...
    if args.once:
        ...
        async def run_once():
            ...
            accepted, verification_score = await _verify_patch(traces, patch)
            if not accepted:
                log.warning(f"Patch for {patch.get('target')} was not approved (score: {verification_score}), exiting.")
                return
            ...
            if _apply_patch(patch):
                ...
                if new_r >= 0:
                    REDIS.lpush(APPLIED_LOG, json.dumps(patch | {"reward": new_r, "verification_score": verification_score}))
                    log.info("Patch applied (once) ✔️  reward=%.4f, verification_score=%.2f", new_r, verification_score)
                else:
