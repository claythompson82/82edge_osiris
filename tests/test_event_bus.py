...
try:
    ...
    fake_aioredis = None 
    FakeServer = None
    logging.warning("fakeredis.aioredis not found. EventBus tests may be skipped or fail.")
...
@unittest.skipIf(fake_aioredis is None, "fakeredis.aioredis is not installed, skipping EventBus tests")
class TestEventBus(unittest.IsolatedAsyncioTestCase):

    async def asyncSetUp(self):
        self.fake_server = FakeServer()
        # Using from_server ensures we are using a server instance we control
        self.fake_redis_client = fake_aioredis.FakeRedis.from_server(self.fake_server, decode_responses=True)
    ...
    async def test_publish_subscribe_single_handler(self):
        ...
        try:
            ...
            self.fail(f"Handler {handler_id} did not receive message for {event_type} in time.")
    ...
    async def test_publish_subscribe_multiple_handlers_same_event(self):
        ...
        try:
            ...
            self.fail(f"One or more handlers did not receive message for {event_type} in time.")
    ...
    async def test_multiple_event_types_and_isolation(self):
        ...
        try:
            await asyncio.wait_for(self.handler_events[handler_id_A].wait(), timeout=1.0)
            await asyncio.wait_for(self.handler_events[handler_id_B].wait(), timeout=1.0)
        except asyncio.TimeoutError:
            self.fail("Handlers for event_A or event_B did not receive messages in time.")
    ...
    async def test_handler_for_one_event_not_receive_other_events(self):
        ...
        try:
            await asyncio.wait_for(self.handler_events[handler_id_Y].wait(), timeout=1.0)
        except asyncio.TimeoutError:
            self.fail(f"Handler {handler_id_Y} for {event_type_Y} did not receive message in time.")
        ...
        self.assertEqual(len(self.received_messages[handler_id_X]), 0, 
                         f"Handler {handler_id_X} for {event_type_X} should not have received messages for {event_type_Y}.")
        self.assertFalse(self.handler_events[handler_id_X].is_set(),
    ...
    async def test_graceful_shutdown(self):
        ...
        self.assertTrue(len(self.event_bus._listener_tasks) > 0, "Listener task should have been created.")
        
        await self.event_bus.close()
        
        self.assertTrue(self.event_bus._stop_event.is_set(), "Stop event should be set after close.")
        ...
        self.assertEqual(len(self.event_bus._listener_tasks), 0, 
        ...
        with self.assertLogs(logger='llm_sidecar.event_bus', level='ERROR') as log_capture:
            ...
            self.assertTrue(any("Error publishing event" in record.getMessage() for record in log_capture.records),
                            "Should log an error when publishing after close.")
        
        # Handler should not have received the message sent after close
        self.assertEqual(len(self.received_messages[handler_id]), 0, 
    ...
    async def test_connection_error_on_connect(self, mock_ping):
        ...
        new_event_bus.redis_client = fake_aioredis.FakeRedis.from_server(self.fake_server, decode_responses=True)
        # Now, specifically patch ping on this instance for the test
        new_event_bus.redis_client.ping = AsyncMock(side_effect=RedisError("Simulated Redis connection error"))
    ...
    async def test_publish_error_handling(self):
        ...
        self.event_bus.redis_client.publish = AsyncMock(side_effect=RedisError("Simulated publish error"))
        
        with self.assertLogs(logger='llm_sidecar.event_bus', level='ERROR') as log_capture:
            await self.event_bus.publish("error_test_event", "test_payload")
            self.assertTrue(any("Error publishing event" in record.getMessage() and "Simulated publish error" in record.getMessage() for record in log_capture.records),
    ...
    async def test_subscribe_after_close_fails_or_logs(self):
        ...
        with self.assertLogs(logger='llm_sidecar.event_bus', level='ERROR') as log_capture:
            ...
            # A more direct test:
            self.assertTrue(self.event_bus._stop_event.is_set(), "Stop event should remain set.")
            ...
            self.assertTrue(any("PubSub not initialized" in r.getMessage() or "Error in listener" in r.getMessage() for r in log_capture.records),
