...
mock_phi3_success_response_data = {
    "ticker": "TEST", "action": "adjust", "side": "LONG",
    "new_stop_pct": 0.05, "new_target_pct": 0.1,
    "confidence": 0.8, "rationale": "Test rationale from phi3"
}
...
class TestOrchestratorCLI(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_cli_successful_run(self, mock_log_run, MockEventBus, mock_requests_post):
        # --- Setup Mocks ---
        ...
        self.assertTrue("http://localhost:8000/generate?model_id=phi3" in phi3_call_args[0][0])
        self.assertTrue("http://localhost:8000/propose_trade_adjustments" in pta_call_args[0][0])
        ...
        expected_phi3_event_payload = json.dumps(mock_pta_success_response_data["phi3_proposal"])
        expected_assessment_event_payload = json.dumps({
            "proposal_transaction_id": mock_pta_success_response_data["transaction_id"],
        ...
        ], any_order=False) # Sequential in current implementation
        ...
        if logged_run_data.final_output: # final_output is Optional[Dict]
             ...
             self.assertEqual(logged_run_data.final_output.get("data"), mock_pta_success_response_data)
        else:
        ...
        self.assertTrue(mock_pta_success_response_data["transaction_id"] in cli_output_str)
        self.assertTrue(mock_pta_success_response_data["hermes_assessment"] in cli_output_str)
        try:
    ...
    @patch('osiris_policy.orchestrator.requests.post')
    @patch('osiris_policy.orchestrator.EventBus') # Mock EventBus to prevent actual calls
    @patch('osiris_policy.orchestrator.log_run')
    async def test_cli_phi3_http_error(self, mock_log_run, MockEventBus, mock_requests_post):
        # --- Setup Mocks ---
        ...
        captured_output = io.StringIO()
        with redirect_stdout(captured_output), self.assertLogs(logger='osiris_policy.orchestrator', level='ERROR') as log_watcher:
            await policy_orchestrator.main_async(test_query)
        ...
        self.assertTrue(any("HTTP request to Phi-3 failed" in msg for msg in log_watcher.output))


    @patch('osiris_policy.orchestrator.requests.post')
    @patch('osiris_policy.orchestrator.EventBus')
    @patch('osiris_policy.orchestrator.log_run')
    async def test_cli_event_publish_error(self, mock_log_run, MockEventBus, mock_requests_post):
        # --- Setup Mocks ---
        mock_event_bus_instance = MockEventBus.return_value
        mock_event_bus_instance.connect = AsyncMock()
        mock_event_bus_instance.publish = AsyncMock(side_effect=RedisError("Simulated Redis Publish Error"))
        ...
        self.assertEqual(logged_run_data.status, "SUCCESS") 
        self.assertIsNotNone(logged_run_data.error_message)
        self.assertTrue("RedisError during event publishing" in logged_run_data.error_message)
        ...
        if logged_run_data.final_output: # for type checker
             ...
             self.assertTrue("RedisError during event publishing" in logged_run_data.final_output["event_publishing_error"])
        ...
        # 4. Check logs
        self.assertTrue(any("RedisError during event publishing" in msg for msg in log_watcher.output))


    @patch('osiris_policy.orchestrator.requests.post')
    @patch('osiris_policy.orchestrator.EventBus')
    @patch('osiris_policy.orchestrator.log_run') # Target where it's imported
    async def test_cli_log_run_error(self, mock_log_run, MockEventBus, mock_requests_post):
        # --- Setup Mocks ---
        mock_event_bus_instance = MockEventBus.return_value
        mock_event_bus_instance.connect = AsyncMock()
        mock_event_bus_instance.publish = AsyncMock()
        ...
        self.assertTrue(any("Failed to log run" in msg and "Simulated DB error" in msg for msg in log_watcher.output))
