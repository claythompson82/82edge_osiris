...
_table_instances = {} # Cache for table instances: {path_and_table_name: table_instance}
...
def init_advice_table(db_path_str: str = DEFAULT_LANCEDB_PATH, table_name: str = "advice"):
...
def log_decision(advice_log_entry: AdviceLog, db_path_str: str = DEFAULT_LANCEDB_PATH, table_name: str = "advice"):
...
def accept(
    ...
    if current_nav <= 0: # NAV should be positive
         return {"accepted": False, "reason": f"Current NAV is non-positive ({current_nav}). Cannot assess risk."}
    ...
    if current_pnl_pct <= -max_daily_loss_pct: # If P&L is already -3% and limit is -2%, reject.
        return {
            "accepted": False,
...
if __name__ == '__main__':
    ...
    # Mock data
    mock_proposal_ok = {"ticker": "TEST", "action": "BUY", "quantity": 10, "price_estimate": 100.0, "run_id": "run_123"} # value = 1000
    mock_proposal_too_large = {"ticker": "TEST", "action": "BUY", "quantity": 20, "price_estimate": 100.0, "run_id": "run_124"} # value = 2000
    
    mock_nav = 10000.0
    mock_pnl_ok = 100.0 # Positive P&L
    mock_pnl_breached = -300.0 # P&L is -3% of NAV (10000 * -0.02 = -200 limit)

    mock_risk_config = {"max_position_size_pct": 0.15, "max_daily_loss_pct": 0.02} # Max 15% position, max 2% daily loss
    ...
    if decision2["accepted"]: # Should not happen for this test
        ...
        log_entry2_rejected = AdviceLog(
            run_id=mock_proposal_too_large.get("run_id", str(uuid.uuid4())), # Ensure run_id
            proposal=mock_proposal_too_large, 
            accepted=decision2["accepted"], 
            reason=decision2["reason"],
            nav_before_trade=mock_nav,
    ...
    try:
        ...
        print("You might need to install 'pyarrow' (pip install pyarrow) to properly query LanceDB tables for verification.")
