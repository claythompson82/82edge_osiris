...
MICRO_LLM_MODEL_PARENT_DIR = "/app/models/llm_micro" # Directory where phi3.onnx is located
MICRO_LLM_MODEL_PATH = os.getenv("MICRO_LLM_MODEL_PATH", os.path.join(MICRO_LLM_MODEL_PARENT_DIR, "phi3.onnx"))
...
def load_phi3_model():
    ...
    print(f"Phi-3 ONNX model will use provider: {'CUDAExecutionProvider' if device == 'cuda' else 'CPUExecutionProvider'}")
    ...
    try:
        ...
        if not os.path.exists(MICRO_LLM_MODEL_PATH):
            print(f"Error: ONNX model file not found at {MICRO_LLM_MODEL_PATH}. Ensure it has been downloaded and named correctly.")
            # Attempt to load from the parent directory anyway, Optimum might find other .onnx files.
             ...
             print(f"Warning: MICRO_LLM_MODEL_PARENT_DIR '{model_dir_to_load}' is not a directory. Attempting to load ONNX model directly from '{MICRO_LLM_MODEL_PATH}'. This might fail if config files are separate.")
